...
class Mkrfox:
    ...
    def readReg(self, regName, length):
        sleep(0.05) #Délai sinon ça marche pas
        try:
            return self.i2cbus.read_i2c_block_data(self.i2c_address, self.register[regName], length) #On lit ensuite la valeur qui arrive
        except:
    ...
    def writeReg(self, regName, data):
        try:
            self.i2cbus.write_i2c_block_data(self.i2c_address, self.register[regName], data)
        except:
